...
| ID | Requirement | Detail |
|---|---|---|
| FR12.1 | PMD Generation | When a visit transitions to "Completed" status, auto-generate a PMD file. **V1 narrowed snapshot (decision #6, 2026-06-12):** patient demographics (name, DOB, sex), Safety Floor (allergies, current medications, active conditions), and visit details (procedures with CDT codes, prescriptions with RxNorm codes). The PMD is **checksum-sealed (SHA-256, tamper-evident)** — digital signing (FR12.4) is honestly deferred to Phase-2, **not** V1. Stored in visit folder. Regenerable if amendments are added (new PMD supersedes previous, linked via provenance). **Phase-2 (deferred from V1):** full identifiers, structured ICD-10 diagnoses, and free-text clinical notes. |
