...
| **G2** | **Fee schedule configured in UI drives no pricing** (AC-ORG-002 unmet). FE `fee-schedule.tsx:66` writes `settings.feeSchedule`; nothing consumes it — `createDentalTreatment.ts:69` takes `priceCents` from the request body; no FE prefill. | Live `v2_5_fee_saved.png`. | **[NEEDS CONFIRMATION]** drive-pricing vs reference-only relabel. |
| **G3** | **Granular permission grid is editable but unenforced.** `assertPermission`/`resolvePermission` (`handlers/shared/assert-permission.ts`) called by **0** production files; real gate is coarse `assertBranchRole` (109 files). | Repo-wide grep. | **[NEEDS CONFIRMATION]** enforce vs remove/disable the tab. |
