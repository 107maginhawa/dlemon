...
| G2 | ✅ **FIXED 2026-06-09 (Batch 4, decision §8 = RELABEL).** Per-patient `PersonConsent` is the enforced send-time gate; the branch panel is relabeled as clinic-wide **defaults/preferences** (intro reworded, toggles described as "Default: …", section header "Default Notification Types") with a prominent `NOTIFICATION_CONSENT_NOTICE` banner making the real gate explicit ("…governed by that patient's communication consent on their profile…"). No new cross-module send-gate was added (no false "master switch"). ~~Settings save but are never enforced.~~ | FE↔BE / trust | **P1** | (was) toggling "Appointment Reminders" off still sent them — misleading. | (resolved) |
