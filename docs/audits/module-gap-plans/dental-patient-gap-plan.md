...
| ~~**G5**~~ | **✅ FIXED 2026-06-09 (Batch 2).** Audit-row assertions added: `approveTreatmentPlan` → `treatment_plan.approved` (+ before/after status, audit added in the same batch); `updateClaimStatus` → `claim.status_changed` (+ before/after, audit added); consent change → characterization pin on `patient.communication_consent.updated`. Proven in `approveTreatmentPlan.test.ts`, `dental-patient-insurance.test.ts`, `communication-consent.test.ts`. See MASTER-GAP-MATRIX §9. | Test gap (audit) | Audit trail for sensitive changes is unverified — could silently regress | grep `audit` across suite |
