...
| ✅P1 | dental-audit | P1-C | **FIXED 2026-06-09 (Batch 2).** Added a per-call `failClosed` opt to `logAuditEvent` (surgical — default fire-and-forget for the other ~70 producers unchanged; NO global flip). Set `failClosed:true` on void/discount/payment/visit-complete/role-change → an audit-sink failure now surfaces a 5xx instead of a silent committed-without-audit gap. | `[xmod]` all producer handlers (roll out per-handler) | move `logAuditEvent` inside tx OR fail-closed for void/discount/payment/visit-complete/role-change | **DONE**: `audit-write-reliability.test.ts` (spyOn the audit insert → void returns 5xx, RED-before) + `updateMember.test.ts` (role-change 5xx). |
...
| **4 — Split-brain config enforcement** *(decisions resolved 2026-06-09 — see §6 #4/#5/#6/#8)* | dental-org, dental-scheduling, notifs, retention | dental-org G1(+G1-shape)/SCH-G3; dental-org G2(+R1); dental-org G3; notifs G2; retention G2 | The "saved-but-not-enforced" class. **Now unblocked:** #6→working-hours canonical `{enabled,open,close}`; #5→fee schedule drives pricing (dedicated endpoints canonical); #4→remove permission-grid tab; #8→relabel notif settings (PersonConsent is the gate). Each fix lands with a **downstream-effect** test (no toast-only). | shape-conformance unit (P0 gate); E2E out-of-hours booking rejected + walk-in bypass; fee→invoice default; permission-grid tab-absent + orphan-route guard; notif panel relabel + per-patient consent surfaced; retention last-run summary |
