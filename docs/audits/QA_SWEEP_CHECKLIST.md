...
- ⚠ **ISSUE-031 (NEW — backend test-infra debt, NOT a product bug)** — the **backend** suite is RED on `main`: **4497 pass / 160 fail in 3 files** (`dental-imaging-events.test.ts`, `imaging-integration.test.ts`, `retention-policy.repo.test.ts`). **Proven pre-existing** (identical **4496 / 160, same 3 files** at baseline `531ffb5e`; unrelated to s6 work — zero file/table overlap). Root: (a) imaging test **teardown FK-ordering** — `delete from imaging_study_image` violates the `imaging_ceph_report_image_id_*` FK when `ceph_report` rows from a sibling test still reference the images (cross-file batch pollution); (b) retention **seed-isolation** leakage (`findEnabled` expects `['clinical']` but sees rows from other tests). Both only fail in the **full-batch shared-DB run** (each file passes/fails deterministically but the harness shares a clone). Test-harness issue, but it **masks real regressions** in those modules → worth fixing. Fix = delete child→parent in teardown (ceph_report before images) or per-test truncate; isolate the retention seed.
