...
- ✅ **ISSUE-044 (PP-7 sub-slice 3) — Occlusion screening.** `createOcclusionScreening`/`listOcclusionScreenings` (create + list only — **no HTTP PATCH/single-GET** exposed; list is a `{data,pagination}` envelope; patient-scoped `…/patients/:id/occlusion-screenings`, module `dental-clinical-ops`) existed with **zero FE references**. Added a `useOcclusionScreenings` hook (list + create; envelope-tolerant; errors via `toastError`) + an `OcclusionScreeningSheet` (list past screenings as Angle-class + finding badges; add form = Angle class / overjet / overbite / crossbite·crowding·spacing / midline / notes — 9 fields, no FSM) opened from a new **Occlusion** trigger in the year-filter bar beside Perio. **Live-verified** (New Screening → Class III + Crossbite + Overjet 3mm → row renders with the badges; no unexpected console errors). +5 unit assertions; FE suite **2644/0**; typecheck + lint + font ratchet (346) clean. `visitId` auto-link scoped out (sheet is patient-scoped; field stays nullable). **Completes PP-7** (alert/task/occlusion shipped; consultation descoped).

- ✅ **ISSUE-045 (PP-9, was ISSUE-018) — iPad-portrait 768px sidebar/billing overflow.** At exactly 768px `useIsMobile` is false (`innerWidth < 768`), so the shadcn sidebar stays in-flow and reserves ~255px — but `SidebarInset` kept its `w-full` (full-viewport) width and was pushed right by the sidebar, so the dashboard widened to **1024px** and the billing table's Status column ran off-screen. **Decision (you chose): keep the sidebar, contain the content** (surgical, no shared `@monobase/ui` breakpoint change). Two-part fix: (1) `<SidebarInset className="min-w-0">` in `_dashboard.tsx` lets the inset shrink to the remaining space (fixes every dashboard page, mirrors the existing inner-`main` min-w-0 calendar fix) — dropped overflow 1024→786; (2) the billing summary cards `grid-cols-3` → `grid-cols-1 md:grid-cols-2 lg:grid-cols-3` (the wide peso totals don't fit 3-across at ~768 with the sidebar) — cleared the residual 18px. The invoices table was already wrapped in `overflow-x-auto min-w-[860px]`, so it now scrolls within its panel (Status reachable). **Live-verified:** 768px → `docSW=768`, no page overflow, 2-up cards, table scrolls to reveal Status; **desktop 1280 unaffected** (3-up cards, full table, normal sidebar, no overflow). CSS-only (no unit test — layout isn't assertable in happy-dom; verified live). FE suite **2644/0**; typecheck + font ratchet (346) clean. **Completes the product-pass backlog (PP-1…9).**
