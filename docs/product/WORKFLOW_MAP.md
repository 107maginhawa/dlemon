...
| WFG-006 | ~~Missing workflow~~ Backend implemented (V-DG-002) | GDPR patient erasure — anonymize-on-request workflow in `handlers/erasure/` (Person+Patient targets, two-step audited, legal-hold blocks). Remaining: HTTP endpoint, more entity targets, real LegalHold store | RESOLVED (backend) | — |
