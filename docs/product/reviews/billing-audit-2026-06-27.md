...
- **Item 9 — SHIPPED:** `collectedThisMonth` now buckets by `paidAt` (server stamps it on invoice close, nulls on void) instead of invoice `createdAt`. Cheap FE fix — `paidAt` was already on the wire (`listDentalInvoices.ts:68`). Correct for full payments (clinic norm); partial-payment-accurate recognition still wants a server payments-by-date KPI (documented upgrade path).
...
- **S-A SKIPPED** (standalone Present/Print Estimate): the requirement is "at minimum collect a deposit"; S-E's always-actionable "Collect Deposit" + the existing advance deep-link kill the dead-end. Print-estimate would be unrequested scope.

### §g EXPERT REVIEW #1 (independent, pre-impl, 2026-06-27) — verdict: SOUND-WITH-REQUIRED-CHANGES. Resolutions folded into S-B/C/D:
- **F-01 (P0 money-leak) → S-D.** Cash-refunding a deposit whose cash was mirrored to credit leaves the patient holding both. **Fix:** in `refundDentalPayment`, when the payment's invoice is `kind='deposit'`, reverse the mirrored deposit credit inside the locked tx (insert `source='deposit_reversed'`, −amount) capped at the patient's *current available* credit; **block (422 DEPOSIT_ALREADY_APPLIED) if available ≤ 0** (already applied → refund from the performed invoice instead). Conserves money in the common single-credit-source case; multi-source-credit precision flagged as a known simplification.
- **F-02 (P1 RLS) → S-B/S-D.** Credit wallet is intentionally patient-global (`getPatientCredits` reads on `db`). Arm `dental_patient_credit` **branch-scoped** (consistent with all armed tables); **all `getBalance`/`listByPatient` reads in the money tx (`applyCreditToInvoice:67`, `refundDentalPayment:123`) switch from `tx`→`db`** so the wallet stays cross-branch. WRITES stay on `tx` (branch in scope).
- **F-03 (P1) → S-C.** Deposit invoice = **due-on-receipt**: create with `dueDate=now()` so `issueDentalInvoice` skips CDT-terms resolution (deposit line has no CDT) and never goes overdue.
- **F-04 (P1 revenue) → S-C.** Exclude `kind='deposit'` from: `getInvoicesForKpis` (billing-report.facade.ts:71, `billedTotalCents`), `getOutstandingInvoicesForAging` (:33-35, AR aging), `getPatientBalance` (:48-50, `totalBilledCents`). Cash-collected must count deposit *payments* once but NOT the credit-application `addPayment` on the performed invoice → compute collected from `dental_payment` rows where feasible; audit `collectedThisMonth`/`dailyCollectionsCents`.
- **F-05 (P1 RLS gate) → S-B.** Arm `dental_patient_credit` AND `dental_payment_refund` (PHI, touched by the deposit refund path, also currently unarmed); add both to `EXPECTED_RLS_TABLES`.
- **F-06 (P2 idempotency) → S-C.** `createDentalDepositInvoice` takes `localId`; check `findByLocalId` before insert (reuse the standard-invoice pattern + the existing `(branch,localId)` partial-unique backstop).
- **F-07 (P2) → S-C.** `applyCreditToInvoice` rejects a `kind='deposit'` target invoice (no self-application).
- Confirmed sound as-designed: FSM untouched (no `markTreatmentsAsBilled`), single-OR-per-peso BIR lifecycle correct, advisory-lock reuse correct.
