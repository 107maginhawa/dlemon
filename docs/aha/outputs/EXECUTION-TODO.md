...
- [~] **dental-pmd Batch C** — partial `unique(visit_id) WHERE generated` + upsert/FOR UPDATE (race); add-only safety-floor merge; **finish mig-0063** (add Drizzle model `imported_pmd_safety_floor_events` + switch merge to append-only INSERT events, decision #20). RED: concurrent double-completion → exactly one generated PMD. (schema fix #4)
  ...
  - [ ] **C2 — DEFERRED (decided 2026-06-12)** mig-0063 finish (Drizzle model `imported_pmd_safety_floor_events` + append-only INSERT-event merge, decision #20). **Rationale:** mechanism-only leaves `markSafetyFloorMerged` at 0 callers (doesn't close the "imported safety data inert" P1 it targets); do the mechanism **with** its consumer when FIX-003 (clinical safety-floor merge) is scoped, as one slice. The orphan-table reconciliation (db:generate would re-CREATE the 0063 table → needs `CREATE TABLE IF NOT EXISTS` / snapshot reconcile) bundles with the **0069 corrective migration + migration-safety prompt** (Track 1 last item / Track 4). Until then the 0063 table stays orphaned (harmless; Drizzle's snapshot doesn't track it, so it doesn't disturb db:generate — confirmed in C1).
- [~] **data-governance Batch C** — retention read API (FR8.14) + erasure/legal-hold TypeSpec role fix `["user"]→["admin"]` (own TypeSpec→regen). Contract pins: GET retention status 200 admin / 403 non-admin. (schema fix #5, cross-cutting fix #4)
  - [x] **retention read API (FR8.14)** DONE 2026-06-12: new `dental-retention.tsp` (`getRetentionStatus` GET `/dental/retention-status`, admin, tag `Retention`→`handlers/retention/`) wrapping existing `summarizeRetentionEnforcement`; handler + 4 RED→GREEN tests (401/403/200 never-run/200 enforced); SDK regen; **contract 6/6** (admin 200 / non-admin 403 verified end-to-end through generated `authMiddleware({roles:["admin"]})`); api-ts+root typecheck 0.
  - [ ] **erasure/legal-hold role fix** `#["user"]→#["admin"]` (8 ops; all handlers already enforce admin → contract-correctness, no behavior change; verified `requestErasure` also admin-gated). Pending.
