...
**Maintained path:** `docs/context/IDEAL_DENTAL_MODULE_WORKFLOW_STANDARD.md` (this file; now a tracked, reconciled standard)  
...
> **Reconciled against implementation 2026-06-08** — see `docs/audits/MODULE_WORKFLOW_AUDIT_2026-06-07.md`.
> This document began life as an *aspirational* target standard written as if much of it
> was unbuilt. A full, code-verified audit established that the platform is **more complete
> than this doc originally implied**: all 28 modules (9 base + 3 healthcare + 16 dental) are
> implemented end-to-end (TypeSpec → handler → route → OpenAPI path), and most items the doc
> tagged `V2 / Deferred` are in fact shipped. The standard's *targets, principles, and rule
> registries remain the canonical reference*; what has been corrected are the **status/factual
> claims** about what exists. Each context table below now carries an **Implementation status**
> note distinguishing what is built from the genuinely-deferred backlog (see §3.14 for the
> consolidated deferred list). Where a row asserted something was missing or future and it is
> actually implemented, the implementation is cited by handler dir or audit reference.
...
**Implementation status: IMPLEMENTED.** Org/branch/membership/role/PIN management, clinic settings, fee schedule, working hours, dashboard summary, and the permission grid all live in `services/api-ts/src/handlers/dental-org/`. Org switcher / membership UX is built in the frontend. Multi-branch is structurally present (branch-scoped access guards in `handlers/shared/`); the "advanced enterprise admin" row (department-level permissions, complex approval chains) remains the deferred part.
...
**Implementation status: IMPLEMENTED (portal in phases).** Patient profile, search (full-name search fixed app-wide), guardian/contacts, medical + dental alerts, consent records (FSM `pending → signed | revoked`), and the patient timeline are all built (`handlers/dental-patient/` + `handlers/patient/`). Patient merge has duplicate-detection (`duplicate-detection.test.ts`) but the merge-with-audit-trail action itself remains deferred (BR-020). **Patient portal Phase 1 IS implemented** — `handlers/dental-portal/` ships `GET /me/appointments`, `/me/invoices`, `/me/balance` behind an `assertSelfPatient` self-scope primitive. *Deferred (Phase 2):* `/me/visits`, `/me/treatment-plans`, `/me/imaging` reads and online payments (need a payments vendor + a PHI-read-scope product decision).
...
**Implementation status: IMPLEMENTED.** Appointment CRUD, calendar (day/week with concurrent-appointment column packing), walk-in flow, visit lifecycle FSM (`draft → active → completed → locked`), provider assignment, queue board (`listQueueBoard`), and waitlist (`createWaitlistEntry`/`promoteWaitlistEntry`) all live in `handlers/dental-scheduling/` + `handlers/dental-visit/`. Online/public booking + appointment-confirmation reminders are also present. The fully-automated SMS/email reminder *campaigns* row remains the deferred part.
...
**Implementation status: IMPLEMENTED.** Encounter/visit records, chief complaint, clinical notes (draft/sign with addendum-based amendment, not silent edit), occlusion screening (`handlers/dental-clinical/occlusion/`), diagnosis entry, provider signature/finalization, and medical-history review are built across `handlers/dental-clinical/` and `handlers/dental-visit/`. The "advanced specialty templates" row remains the deferred part.
...
**Implementation status: IMPLEMENTED — including the two rows tagged `V2 / Deferred`.** Odontogram (permanent + pediatric dentition), per-tooth and per-surface charting, the separate baseline/proposed/completed layers, visual status legend, and tooth history are all built. **Advanced periodontal charting IS implemented and full-stack** (`handlers/dental-perio/`; editable probing grid + voice charting `use-voice-perio.ts` + longitudinal multi-exam comparison `perio-comparison.tsx`; AAP/EFP 2017 staging+grading; E2E journey). **Cephalometric workspace IS implemented (v1.4)** — landmarking, tracing overlays, 6 analyses, superimposition, manual UX (loupe/keyboard/Ricketts switcher) in `handlers/dental-imaging/Ceph*`. The only deferred ceph piece is AI auto-tracing (an intentional non-goal — product is local-first/no-AI).
...
**Implementation status: IMPLEMENTED.** Treatment plan + items, the status FSM (`draft → presented → approved → partially-completed → completed → cancelled`), patient approval capture, estimates, sequencing/phasing (`treatment-phasing.test.ts`), convert-to-work-done, and option groups are built in `handlers/dental-patient/treatment-plans/`. **Case presentation IS implemented** (`handlers/dental-patient/case-presentation/`) and is now gated by the new `treatment_coordinator` role. The "before/after simulation" flavour of advanced case presentation remains the deferred part.
...
**Implementation status: IMPLEMENTED.** Completed procedures with the treatment FSM (`diagnosed → planned → performed → verified`, forward-only, performed is immutable), work-done-from-plan transition, direct same-day work with audit, clinical-note linkage, materials usage (via inventory), and addendum/reversal corrections are all built across `handlers/dental-visit/treatments/` and `handlers/dental-clinical/`. The "specialty procedure records" row remains the deferred part.
...
**Implementation status: IMPLEMENTED.** Invoice creation (from completed procedures + manual charges), invoice items, payment recording (multi-method), receipts, patient balance, role-restricted discounts, void/mark-uncollectible (owner-only write-off, BR-013), and AR/unpaid lists are built in `handlers/dental-billing/`. Invoice FSM (`draft → issued → paid|partial → paid|overdue`, plus `voided`/`uncollectible`). The "advanced accounting / GL export" row remains the deferred part.
...
**Implementation status: IMPLEMENTED (drafts; e-submission deferred).** Insurance profiles, coverage authorization, claim drafts, claim readiness review, and CDT/ICD-10 coding support are all built in `handlers/dental-patient/insurance/` (`createInsuranceProfile`, `createCoverageAuthorization`, `createClaimDraft`, `getClaimReadiness`, `updateClaimStatus`). Attachments are linkable to claim context. *Genuinely deferred:* electronic clearinghouse submission and ERA/EOB remittance processing — the claim draft package is produced but not transmitted electronically.
...
**Implementation status: IMPLEMENTED.** File upload/capture, image categories, patient/visit/tooth/procedure linkage, preview, annotation, and the imaging-finding FSM (`draft → confirmed → resolved`) are built in `handlers/dental-imaging/` (storage via S3/MinIO). Of the "advanced imaging tools" row, **ceph tracing + measurements ARE implemented** (see §3.5); the genuinely-deferred items are AI findings and full DICOM support.
...
**Implementation status: IMPLEMENTED — this context is NOT missing.** Inventory items, stock adjustments (with reason), material usage, and low-stock flagging are built in `handlers/dental-clinical/inventory/` (`createInventoryItem`, `createInventoryAdjustment`, `listInventoryItems`, `listInventoryAdjustments`, `updateInventoryItem`; spec `dental-clinical-ops.tsp`). The supplier-management and full-inventory-accounting rows remain the deferred parts.
...
**Implementation status: IMPLEMENTED.** Recall scheduling (`handlers/dental-patient/recalls/`), follow-up tasks (`dental-patient-tasks`), patient alerts, post-op instructions (`handlers/dental-clinical/postop/`), and manual patient-communication notes are built. The fully-automated messaging and CRM/campaign rows remain the deferred parts.
...
**Implementation status: IMPLEMENTED (sync metadata + offline create); conflict UI deferred.** Audit logging (Pino structured logs + `handlers/dental-audit/` + async pg-boss audit-on-write), soft-delete/reversal, and **local-first sync metadata ARE implemented** — `handlers/dental-patient/sync/` ships `createSyncLog`/`listSyncLogs`/`updateSyncLog` (SyncLog/SyncState with local-ID/server-ID mapping and pending/synced/failed status), and there is even an E2E journey (`apps/dentalemon/tests/e2e/journeys/15-offline-sync-metadata.journey.spec.ts`). The CRDT/P2P transport itself exists as the standalone `services/cadence/` engine (embedded into the Tauri wrapper). *Genuinely deferred:* the full human-facing conflict-resolution UI and end-to-end-activated CRDT/P2P sync (the cadence `SyncEngine` integration in the Tauri host is still a stub).

---

### 3.14 Consolidated Deferred / Backlog (the honest open items)

Everything else in §3 is implemented. The following are the genuinely-open items, kept as the
accurate target backlog (cross-referenced to the audit, §8.B/D):

| Deferred item | Notes |
|---|---|
| Patient portal **Phase 2** | `/me/visits`, `/me/treatment-plans`, `/me/imaging` reads + **online payments** (needs payments-vendor selection + PHI-read-scope product decision). Phase 1 reads + `assertSelfPatient` are shipped. |
| Electronic clearinghouse submission / ERA-EOB | Claim drafts + readiness exist; electronic e-submission and remittance processing are deferred. |
| Full local-first conflict-resolution UI | Sync metadata + offline create exist; human conflict-review UI and fully-activated CRDT/P2P sync remain. |
...
**Implementation status: IMPLEMENTED (local create + sync status + reconcile); conflict-resolution UI deferred.** Local-ID creation, sync-status surfacing, local-change preservation, and local-ID→server-ID reconciliation are covered by `handlers/dental-patient/sync/` and the offline-sync E2E journey. Only the user-facing conflict-resolution workflow remains deferred.
...
| Treatment Coordinator | Present treatment plans / case presentations to patients; surface costs + payment options; no clinical writes. | V1 Recommended |
| Read-only / Auditor | View records and audit logs without edit rights. | V1 Recommended |

**Implementation status: IMPLEMENTED.** The role set is built and enforced via membership-role guards (`assertBranchRole`). Note the actual role keys: `dentist_owner`, `dentist_associate`, `hygienist`, `dental_assistant`, `staff_full`/`front_desk`, `staff_scheduling`, `billing_staff`, and the new `treatment_coordinator` (case-presentation gating). This session also wired `dental_assistant` clinical-assist (chart upsert / imaging capture / draft notes / attachments under supervision) and `hygienist` hygiene-led visits scoped by `visitType`. See `docs/product/ROLE_PERMISSION_MATRIX.md` for the authoritative grid.
...
The following should not be forced into V1 unless explicitly required. **Reconciliation note:** two rows below this doc originally listed as non-goals — *advanced perio charting* and the *cephalometric workspace* — have since been **built and shipped** (perio is full-stack with comparison + voice; ceph is v1.4). They were promoted out of "non-goal" status; what remains a deliberate ceph/perio non-goal is **AI auto-tracing / AI voice charting** (the product is local-first and intentionally no-AI). The table below now reflects only the items that are genuinely still non-goals.

| Non-Goal | Reason | Priority |
|---|---|---|
| Full electronic clearinghouse submission / ERA-EOB | Claim drafts + readiness are built; e-submission is deferred unless integration is in scope. | V2 / Deferred |
| ~~Advanced perio charting~~ | **NOW IMPLEMENTED** (full-stack: editable grid + voice + multi-exam comparison + E2E). No longer a non-goal. | Shipped |
| Cephalometric **AI** auto-tracing | The manual ceph workspace IS shipped (v1.4); only AI-assisted landmark auto-tracing is an intentional non-goal (no-AI product stance). | V2 / Deferred |
| Full orthodontic case management | Specialty workflow; defer unless target market requires it. | V2 / Deferred |
| Full inventory accounting | Basic inventory + material tracking IS built; costing/batch/expiry/warehouse controls are deferred. | V2 / Deferred |
| Enterprise analytics | Operational dashboard is more important for V1. | V2 / Deferred |
| Full patient app | Portal Phase 1 reads ARE shipped; the full patient app (Phase 2 reads, messaging, online payments) is deferred. | V2 / Deferred |
| Complex sync conflict UI | Sync metadata + offline create are built; the human conflict-resolution UI is deferred. | V2 / Deferred |
| AI-dependent clinical decisions | Data is AI-ready, but workflows work without AI; AI-dependent decisions are an intentional non-goal. | V2 / Deferred |
