 ...
 */
export function shouldExpand(query: Record<string, unknown> | undefined | null, field: string): boolean {
  const expand = query?.['expand'];
  if (typeof expand === 'string') return expand.includes(field);
  if (Array.isArray(expand)) return expand.includes(field);
  return false;
}
...
export function parsePagination(
  ...
  if (query?.['pageSize'] !== undefined || query?.['page'] !== undefined) {
    ...
    const pageSize = Math.min(
      parseInt(String(query?.['pageSize'] ?? '')) || defaultLimit,
      maxLimit
    );
    const page = Math.max(parseInt(String(query?.['page'] ?? '')) || 1, 1); // Ensure page >= 1
    ...
    limit = Math.min(
      parseInt(String(query?.['limit'] ?? '')) || defaultLimit,
      maxLimit
    );
    offset = Math.max(parseInt(String(query?.['offset'] ?? '')) || 0, 0); // Ensure offset >= 0
  }
 ...
 */
export function parseFilters(query: Record<string, unknown> | undefined | null, allowedFields: string[]): Record<string, unknown> {
...
export function parseSort(
  ...
  const sortParam = (query?.['sort'] as string | undefined) || defaultSort;
