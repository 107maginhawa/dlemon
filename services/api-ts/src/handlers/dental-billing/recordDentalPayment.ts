...
export async function recordDentalPayment(
  ...
  const { payment, updatedInvoice } = await withTenantTx(db, { branchIds: [invoice.branchId] }, async (tx) => {
    ...
    if (invoice.kind === 'deposit') {
      // F-07: take the per-patient credit lock (1001) so a concurrent
      // applyCreditToInvoice/refund can't read a stale wallet between this mirror
      // insert and its commit (would surface a transient false NO_CREDIT).
