...
export async function voidDentalPayment(
  ...
  // §g F-01: voiding a DEPOSIT payment must reverse the mirrored deposit credit —
  // exactly like refundDentalPayment — or the patient keeps the spendable credit
  // while the invoice is restored to unpaid (money from nothing).
  ...
  const voided = await withTenantTx(db, { branchIds: [payment.branchId] }, async (tx) => {
    ...
    // §g F-01: reverse the mirrored deposit credit (capped at the still-available
    ...
    if (isDeposit) {
      ...
      // Read on db (global wallet) while holding the 1001 lock on tx — safe: every
      // credit mutator takes the same per-patient lock, so no competing write commits.
