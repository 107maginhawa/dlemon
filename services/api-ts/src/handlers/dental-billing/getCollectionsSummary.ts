...
export async function getCollectionsSummary(ctx: BaseContext) {
  ...
  const invoiceConditions: SQL<unknown>[] = [
    ...
    // §g DQ3 / F-02 (re-review #2): exclude deposit invoices from billed/
    // outstanding/overdue so the collections summary matches the KPI dashboard.
    // Collected cash is summed from dental_payment rows below, so deposit CASH
    // still counts there once (and the credit-application is not a payment row).
