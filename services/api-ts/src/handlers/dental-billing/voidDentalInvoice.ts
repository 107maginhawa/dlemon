...
export async function voidDentalInvoice(
  ...
  // §g F-04 (re-review #2): voiding a PAID deposit invoice would strand the
  // mirrored deposit credit (the patient keeps spendable credit with no invoice
  // debt). Require the deposit payment to be refunded/voided first — which
  // reverses the credit — before the deposit invoice can be voided.
