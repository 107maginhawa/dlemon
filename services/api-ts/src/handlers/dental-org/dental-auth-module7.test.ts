 ...
 * - Recover PIN with wrong answer → 401
 * - Recover PIN with no security question set → 422
...
describe('POST .../recover-pin (FR9.7)', () => {
  ...
  test('401 with wrong answer', async () => {
    await setupSecurityQuestion();
    const app = buildTestApp();
    const res = await app.request(`/dental/org/members/${MEMBER_ID}/recover-pin`, {
      method: 'POST',
  ...
  test('422 when no security question set', async () => {
    ...
    const body = await res.json() as any;
    expect(body.code).toBe('NO_SECURITY_QUESTION');
  });
