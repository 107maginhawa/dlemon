...
describe('DentalBranchManagement_create handler', () => {
  ...
  test('branch is isolated to its org — listByOrg returns only this org branches', async () => {
    ...
    await orgRepo.createOne({
      ...
      // owner holding two active solo/clinic orgs. This isolation test only needs a
      // SEPARATE org, so a different owner is the faithful seed.
      ownerPersonId: OTHER_ID,
      countryCode: 'PH',
      active: true,
    });
