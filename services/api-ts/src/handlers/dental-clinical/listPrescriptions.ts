...
export async function listPrescriptions(ctx: HandlerContext) {
  ...
  if (audit?.logEvent) {
    await audit.logEvent({ eventType: 'data-access', category: 'clinical', action: 'read', outcome: 'success', user: user.id, userType: 'client', resourceType: 'prescription', resource: visitId, description: 'Prescriptions listed for visit', details: { resultCount: items.length }, ipAddress: ctx.req.header('x-forwarded-for'), userAgent: ctx.req.header('user-agent'), request: ctx.req.header('x-request-id') }, user.id);
  }
