...
export async function listAttachments(ctx: HandlerContext) {
  ...
  if (audit?.logEvent) {
    await audit.logEvent({ eventType: 'data-access', category: 'clinical', action: 'read', outcome: 'success', user: user.id, userType: 'client', resourceType: 'attachment', resource: visitId, description: 'Attachments listed for visit', details: { resultCount: totalCount }, ipAddress: ctx.req.header('x-forwarded-for'), userAgent: ctx.req.header('user-agent'), request: ctx.req.header('x-request-id') }, user.id);
  }
