/**
 * PMD generation trigger — failure isolation (AHA FIX-001).
 *
 * Visit completion is a hot clinical path. The auto-generation trigger must be
 * failure-ISOLATED: if PMD generation throws, the visit must STILL complete
 * (200, status='completed'), because the clinical record + audit row are already
 * committed before the trigger runs. This test forces the generation core to
 * throw (via mock.module) and asserts completion is unaffected.
 *
 * Isolated in its own file — the test runner gives each file its own process, so
 * the mock.module here does not leak into the real-core trigger test.
 */
...
// Force the PMD generation core to throw, so we can prove completion survives it.
mock.module('@/handlers/dental-pmd/generatePMD', () => ({
  generatePmdForVisit: async () => { throw new Error('boom: simulated PMD generation failure'); },
  generatePMD: async () => new Response('{}', { status: 201 }),
}));
...
const TEST_USER = { id: '00000000-0000-0000-0000-0000000ad018', email: 'pmd-fail@clinic.com' };
const ORG_ID = 'ef000000-0000-1000-8000-0000000ad018';
const BRANCH_ID = '7b000000-0000-4000-8000-0000000ad018';
const MEMBER_ID = '7c000000-0000-4000-8000-0000000ad018';
const PERSON_ID = 'f1000000-0000-1000-8000-0000000ad018';
const PATIENT_ID = 'a0000000-0000-1000-8000-0000000ad018';
...
beforeAll(async () => {
  ...
  await db.insert(dentalOrganizations).values({ id: ORG_ID, name: 'PMD Fail Clinic', tier: 'solo', ownerPersonId: TEST_USER.id, countryCode: 'PH', createdBy: TEST_USER.id, updatedBy: TEST_USER.id }).onConflictDoNothing();
...
describe('PMD trigger failure isolation (FIX-001)', () => {
  test('visit completion succeeds (200, completed) even when PMD generation throws', async () => {
    const app = buildApp();
    ...
    // Completion is unaffected by the PMD failure.
    expect(res.status).toBe(200);
    expect(((await res.json()) as any).status).toBe('completed');

    // And no PMD row was produced (generation threw and was swallowed).
    const rows = await db.execute(sql`SELECT count(*)::int AS n FROM pmd_document WHERE visit_id = ${visit.id}`);
