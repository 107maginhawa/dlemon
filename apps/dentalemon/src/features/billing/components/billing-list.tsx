      ...
      >
        {FILTER_TABS.map((tab) => (
          <button
            ...
            className={`h-[30px] px-3.5 rounded-lg text-sm font-medium tracking-tight transition-colors ${
              activeTab === tab
      ...
      <div className="bg-background rounded-2xl shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[860px] border-collapse">
            <thead>
              <tr>
                <th className="text-left text-xs font-semibold tracking-wider uppercase text-muted-foreground px-4 py-3 border-b border-border pl-5">
                  Invoice #
                </th>
                <th className="text-left text-xs font-semibold tracking-wider uppercase text-muted-foreground px-4 py-3 border-b border-border">
                  Patient
                </th>
                <th className="text-left text-xs font-semibold tracking-wider uppercase text-muted-foreground px-4 py-3 border-b border-border">
                  Visit Date
                ...
                <th className="text-right text-xs font-semibold tracking-wider uppercase text-muted-foreground px-4 py-3 border-b border-border pr-5">
                  Actions
                </th>
              </tr>
            </thead>
              ...
              {!loading &&
                invoices.map((inv) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-0 h-12 align-middle text-sm font-medium">
                      {inv.patientName ?? inv.patientId}
                    </td>
                    <td className="px-4 py-0 h-12 align-middle text-sm tabular-nums">
                      {inv.visitDate ?? '--'}
                    </td>
                    <td className="px-4 py-0 h-12 align-middle text-sm font-semibold tabular-nums">
                      {formatCents(inv.totalCents)}
                    </td>
                    <td className={`px-4 py-0 h-12 align-middle text-sm tabular-nums ${getBalanceClass(inv.balanceCents)}`}>
                      {formatCents(inv.balanceCents)}
                    </td>
                    <td className="px-4 py-0 h-12 align-middle">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold ${getStatusBadgeClass(inv.status)}`}>
