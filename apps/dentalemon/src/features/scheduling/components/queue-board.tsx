...
const COLUMN_HEADER_CLASS: Record<string, string> = {
  ...
  in_progress: 'bg-accent text-lemon-foreground border-lemon',
    ...
    <div className="rounded-lg border border-border bg-background p-3 shadow-sm flex flex-col gap-2">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{timeWaiting(item.createdAt)}</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 flex-wrap">
        {primaryAction && (
          <button
            ...
            className="rounded px-2.5 py-1 text-xs font-semibold bg-lemon text-lemon-foreground hover:bg-lemon-hover disabled:opacity-50 transition-colors"
          >
            {primaryAction.label}
          </button>
        )}
