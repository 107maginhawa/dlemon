...
describe('PatientFilterTabs', () => {
  ...
  test('calls onFilterChange with "active" when Active is clicked', async () => {
  ...
  test('calls onFilterChange with "needs-follow-up" when Follow-Up is clicked', async () => {
    const user = userEvent.setup();
    let received: PatientFilter | null = null;
    render(React.createElement(PatientFilterTabs, {
      activeFilter: 'all',
    ...
    await user.click(screen.getByRole('tab', { name: /Follow/ }));
    expect(received).toBe('needs-follow-up');
  });

  test('calls onFilterChange with "archived" when Archived is clicked', async () => {
    const user = userEvent.setup();
    let received: PatientFilter | null = null;
    render(React.createElement(PatientFilterTabs, {
      activeFilter: 'all',
  ...
  test('shows count badge when count is provided', () => {
    ...
    expect(screen.getByText('47')).not.toBeNull();
